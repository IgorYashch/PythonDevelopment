    ...
    while not reader.at_eof() and not quit:
        ...
        for q in done:
            if q is send:
                ...
                if result == 'quit':
                ...
                elif result.startswith('cows'):
                    used_cows = set(cow for cow, _ in clients.values() if cow)
                    available_cows = set(list_cows()) - used_cows
                    await clients[me][1].put(f"Available cows: {', '.join(available_cows)}")
                    
                elif result.startswith('login'):
                    _, cowname = result.split()
                    used_cows = set(cow for cow, _ in clients.values() if cow)
                    if cowname in used_cows:
                        await clients[me][1].put("Name already in use")
                    elif cowname in list_cows():
                        clients[me][0] = cowname
                        await clients[me][1].put(f"You logged as {cowname}")
                        login = cowname
                    else:
                        await clients[me][1].put("Unknown cowname")
                        
                elif result == 'who':
                    used_cows = [cow for cow, _ in clients.values() if cow]
                    await clients[me][1].put(f"Authorized cows: {', '.join(used_cows) or 'there is no logged cows'}")
                    ...
                    await clients[me][1].put(f"You are not authorized. Please use: login cowname")
